import numpy as np
from fem import *
from solver import Unsteady_Stokes_Linear_Elasticity_2D_FE_solver_Dirichlet
from basis import Basis

def u1_func(x, y, t, derivative_order_x, derivative_order_y):
    '''
    解析解
    '''
    if derivative_order_x==0 and derivative_order_y==0:
        u = (x**2*y**2 + np.exp(-y))*np.cos(2*np.pi*t)
    elif derivative_order_x==1 and derivative_order_y==0:
        u = 2*x*y**2*np.cos(2*np.pi*t)
    elif derivative_order_x==0 and derivative_order_y==1:
        u = (2*x**2*y - np.exp(-y))*np.cos(2*np.pi*t)
    else:
        ValueError("The derivative order is not correct!")
    return u

def u2_func(x, y, t, derivative_order_x, derivative_order_y):
    '''
    解析解
    '''
    if derivative_order_x==0 and derivative_order_y==0:
        u = (-2/3*x*y**3 + 2 - np.pi*np.sin(np.pi*x))*np.cos(2*np.pi*t)
    elif derivative_order_x==1 and derivative_order_y==0:
        u = (-2/3*y**3 - np.pi**2*np.cos(np.pi*x))*np.cos(2*np.pi*t)
    elif derivative_order_x==0 and derivative_order_y==1:
        u = (-2*x*y**2)*np.cos(2*np.pi*t)
    else:
        ValueError("The derivative order is not correct!")
...
def p_func(x, y, t, derivative_order_x, derivative_order_y):
    '''
    解析解
    '''
    if derivative_order_x==0 and derivative_order_y==0:
        u = -(2-np.pi*np.sin(np.pi*x))*np.cos(2*np.pi*y)*np.cos(2*np.pi*t)
    elif derivative_order_x==1 and derivative_order_y==0:
        u = np.pi**2*np.cos(np.pi*x)*np.cos(2*np.pi*y)*np.cos(2*np.pi*t)
    elif derivative_order_x==0 and derivative_order_y==1:
        u = (4*np.pi*np.sin(2*np.pi*y) - 2*np.pi**2*np.sin(np.pi*x)*np.sin(2*np.pi*y))*np.cos(2*np.pi*t)
    else:
        ValueError("The derivative order is not correct!")
    return u

def source1_function(x, y, t, nu):
    sour = -2*np.pi*(x**2*y**2 + np.exp(-y))*np.sin(2*np.pi*t)+\
           (-2*nu*x**2 - 2*nu*y**2 - nu*np.exp(-y) + np.pi**2*np.cos(np.pi*x)*np.cos(2*np.pi*y))*np.cos(2*np.pi*t)
    return sour

def source2_function(x, y, t, nu):
    sour = -2*np.pi*(-2/3*x*y**3+2-np.pi*np.sin(np.pi*x)) * np.sin(2*np.pi*t) +\
            (4*nu*x*y - nu*np.pi**3*np.sin(np.pi*x) + 2*np.pi*(2-np.pi*np.sin(np.pi*x))*np.sin(2*np.pi*y)) * np.cos(2*np.pi*t)
    return sour

if __name__ == '__main__':

    for i in range(5):
        bounds = [0, 1, -0.25, 0, 0, 1]
        Nx = 8*(2**i)
        Ny = 2*(2**i)
        Nt = 4*(2**i)
        theta = 1/2
        nu = 2
        solver = Unsteady_Stokes_Linear_Elasticity_2D_FE_solver_Dirichlet(bounds, Nx, Ny, Nt, theta, nu=nu, source1_function=source1_function, source2_function=source2_function, \
