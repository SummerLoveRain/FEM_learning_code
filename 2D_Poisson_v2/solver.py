        ...
        def func(x, y):
            ...
            for i in range(Nb):
                ...
                if self.is_in_triangle(x, y, vertices):
                    for j in range(Nl):
                        value += self.uh[Tb[i, j]]*self.basis_function(x, y, vertices, j, derivative_order_x=0, derivative_order_y=0)
                    break
            return value
    ...
    def L_infinity_error(self, u_func):
        ...
        for i in range(Nb):
            ...
            def uh_func(x, y):
                uh = 0.0
                for j in range(Nl):
                    uh += self.uh[Tb[i, j]]*self.basis_function(x, y, vertices, j, derivative_order_x=0, derivative_order_y=0)
                return uh
